// ==UserScript==
// @name               Media Helper: download Instagram pic & vids
// @name:zh-CN         Media Helper: 下载 Instagram 图片和视频
// @version            1.9.17.0
// @namespace          MediaHelper
// @homepage           https://github.com/mittya/media-helper
...
(function() {
  ...
  */
  ...
  var ig_helper_style = '.downloadBtn {' +
    ...
    '}' +
    '.downloadBtn.inStories {width:28px; top:90px; right:22px; border-radius:50%; font-size:12px; background-size:18px;}' +
    '.qbCDp {position: relative;} .KL4Bh:hover .downloadBtn, .OAXCp:hover .downloadBtn, .qbCDp:hover .downloadBtn {opacity:1} ' +
  ...
  */
  ...
  function findMedia(box, way) {
    ...
    _box.addEventListener('mouseover', function(event) {
      ...
      */
      if (event.target.className && typeof event.target.className.indexOf === 'function' && event.target.className.indexOf('fXIG0') >= 0) {
        _parent = event.target.parentNode;
      ...
      /*
        IG TV
      */
      if (event.target.className && typeof event.target.className.indexOf === 'function' && event.target.className.indexOf('tWeCl') >= 0) {
        _parent = event.target.parentNode;
        _url = _parent.querySelector('.tWeCl').src;
        _title = _url.match(/[a-zA-Z0-9_]+.mp4/g);
        _username = '';
      ...
      */
      if (event.target.className && typeof event.target.className.indexOf === 'function' && event.target.className.indexOf('B20bj') >= 0 && _way === 'stories') {
